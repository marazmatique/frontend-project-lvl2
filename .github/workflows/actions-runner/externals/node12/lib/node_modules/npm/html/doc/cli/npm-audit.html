...
common options: [--production] [--only=(dev|prod)]</code></pre>
    ...
    <p>Run <code>audit fix</code> without modifying <code>node_modules</code>, but still updating the
        pkglock:</p>
    <pre><code>$ npm audit fix --package-lock-only</code></pre>
    <p>Skip updating <code>devDependencies</code>:</p>
    <pre><code>$ npm audit fix --only=prod</code></pre>
    ...
    <pre><code>$ npm audit fix --dry-run --json</code></pre>
    ...
    <pre><code>$ npm audit --json</code></pre>
    ...
    <ul>
        <li>Any module referencing a scope that is configured for a non-default
            registry has its name scrubbed. (That is, a scope you did a <code>npm login --scope=@ourscope</code> for.)
        </li>
